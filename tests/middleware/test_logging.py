...
@pytest.mark.asyncio
async def test_trace_logging(caplog, unused_tcp_port):
    config = Config(app=app, port=unused_tcp_port, log_level="trace")
...
async def test_trace_logging_on_http_protocol(http_protocol, caplog, unused_tcp_port):
    config = Config(
        app=app, port=unused_tcp_port, log_level="trace", http=http_protocol
    )
...
async def test_trace_logging_on_ws_protocol(ws_protocol, caplog, unused_tcp_port):
    ...
    config = Config(
        app=websocket_app, port=unused_tcp_port, log_level="trace", ws=ws_protocol
    )
    with caplog_for_logger(caplog, "uvicorn.error"):
        async with run_server(config):
...
async def test_access_logging(use_colors, caplog, unused_tcp_port):
    config = Config(app=app, port=unused_tcp_port, use_colors=use_colors)
...
async def test_default_logging(use_colors, caplog, unused_tcp_port):
    config = Config(app=app, port=unused_tcp_port, use_colors=use_colors)
    with caplog_for_logger(caplog, "uvicorn.access"):
        async with run_server(config):
            async with httpx.AsyncClient() as client:
        ...
        assert f"Uvicorn running on http://127.0.0.1:{unused_tcp_port}" in messages.pop(
            0
        )
...
@pytest.mark.asyncio
async def test_unknown_status_code(caplog, unused_tcp_port):
    async def app(scope, receive, send):
        assert scope["type"] == "http"
