...
def run_loop(loop):
    ...
    # Ensure that we don't leave any Tasks running
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_accept_connection(protocol_cls, loop):
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_send_after_close(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
            await self.send({"type": "websocket.accept"})
            await self.send({"type": "websocket.send", "text": "before_close"})
            await self.send({"type": "websocket.close"})
            await self.send({"type": "websocket.send", "text": "after_close"})
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_send_after_client_disconnects(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
            await self.send({"type": "websocket.accept"})
            while True:
    ...
    async def open_connection(url):
        async with websockets.connect(url) as websocket:
            message = await websocket.recv()
            websocket.writer.transport.close()
            return message

    with run_server(App, protocol_cls=protocol_cls) as url:
        message = loop.run_until_complete(open_connection(url))
        assert message == "123"


@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_receive_after_client_disconnects(protocol_cls, loop):
    disconnects = []
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_receive_client_closes(protocol_cls, loop):
    disconnects = []
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_close_after_client_closes(protocol_cls, loop):
    messages = []
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_close_connection(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
            await self.send({"type": "websocket.close"})
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_headers(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_send_text_data_to_client(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
            await self.send({"type": "websocket.accept"})
            await self.send({"type": "websocket.send", "text": "123"})
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_send_binary_data_to_client(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
            await self.send({"type": "websocket.accept"})
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_send_and_close_connection(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
            await self.send({"type": "websocket.accept"})
            await self.send({"type": "websocket.send", "text": "123"})
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_send_text_data_to_server(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
            await self.send({"type": "websocket.accept"})
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_send_binary_data_to_server(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
            await self.send({"type": "websocket.accept"})
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_send_after_protocol_close(protocol_cls, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
            await self.send({"type": "websocket.accept"})
            await self.send({"type": "websocket.send", "text": "123"})
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_missing_handshake(protocol_cls, loop):
    class App:
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_send_before_handshake(protocol_cls, loop):
    class App:
        def __init__(self, scope):
            pass
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_duplicate_handshake(protocol_cls, loop):
    class App:
        def __init__(self, scope):
            pass
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_asgi_return_value(protocol_cls, loop):
    """
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_app_close(protocol_cls, loop):
    async def app(scope, receive, send):
        while True:
...
@pytest.mark.parametrize("protocol_cls", WS_PROTOCOLS)
def test_client_close(protocol_cls, loop):
    async def app(scope, receive, send):
        while True:
            message = await receive()
            if message["type"] == "websocket.connect":
...
def test_subprotocols(protocol_cls, subprotocol, loop):
    class App(WebSocketResponse):
        async def websocket_connect(self, message):
